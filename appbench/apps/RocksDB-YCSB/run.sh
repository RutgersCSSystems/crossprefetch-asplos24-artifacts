...
BENCHMARK="fillycsb,ycsbwklda,ycsbwkldb,ycsbwkldc,ycsbwkldd,ycsbwklde,ycsbwkldf"
...
RUN() {
	...
	./db_bench --db=$DBPATH --num_levels=6 --key_size=20 --prefix_size=20 --memtablerep=prefix_hash --bloom_bits=10 --bloom_locality=1 --benchmarks=$BENCHMARK --use_existing_db=0 --num=4000000 --duration=10 --compression_type=none --value_size=$2 --threads=$1  #&> $result_dir/$WORKLOADDESCR"_"$1"_"$2".txt"
	sleep 2
}
