...
make -j$(nproc) FLAGS=-DGET_LATENCY NR_RA_PAGES=$PREFETCH_SIZE_VAR NR_WORKERS=$NR_WORKERS_VAR CROSS_BITMAP_SHIFT=$CROSS_BITMAP_SHIFT NR_QSIZE=$NR_QSIZE_VAR NR_NQUEUES=$NR_NQUEUES_VAR
make install
